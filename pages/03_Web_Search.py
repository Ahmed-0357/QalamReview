...
if st.session_state['google_api'] == '' or st.session_state['google_search_engine_id'] == '':
    ...
    st.markdown('')
    st.markdown(
        "🌍 Please enter search terms/search keywords (comma separated)")

    input_search_terms = st.text_input(
        '', placeholder="example: CO2 underground storage, CO2 EOR, CO2 Subsurface injection", label_visibility='collapsed')
    ...
    if st.button('Start Web Search'):
        if input_search_terms.strip() == '':
            st.error("Please enter search terms/search keywords.", icon="🚨")
        else:
            search_terms_list = input_search_terms.strip().strip(
                ',').strip().strip(',').split(',')
